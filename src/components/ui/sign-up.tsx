...
export const SignUpPage: React.FC<SignUpPageProps> = ({
  title = <span className="font-light text-foreground tracking-tighter">Join Belka</span>,
  description = "Create your account and start organizing your travel memories",
  heroImageSrc,
  testimonials = [],
  error,
  isLoading = false,
          ...
          <div className="flex flex-col gap-6">
            ...
            <form className="space-y-5" onSubmit={onSignUp}>
              ...
              <div className="animate-element animate-delay-600 flex items-start gap-3 text-sm">
                <input type="checkbox" name="acceptTerms" className="custom-checkbox mt-1" disabled={isLoading} required />
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="animate-element animate-delay-700 w-full"
                size="lg"
              >
                {isLoading ? (
              ...
              {error && (
                <div className="animate-element animate-delay-800 text-red-500 text-sm text-center bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800/30 rounded-lg p-3">
                  {error}
                </div>
              )}
            </form>
            ...
            <p className="animate-element animate-delay-1100 text-center text-sm text-muted-foreground">
              Already have an account? <a href="#" onClick={(e) => { e.preventDefault(); if (!isLoading) onSignIn?.(); }} className={`text-violet-400 hover:underline transition-colors ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>Sign in</a>
            </p>
          </div>
        </div>
      </section>
